            ...
            @if (isset($templates['plan'][0]) && ($plan = $templates['plan'][0]))
                <div class="row">
                    <div class="section_header text-center pb-5">
                        <h2>@lang(optional($plan->description)->title)</h2>
                ...
                @forelse($plans as $key => $plan)
                    <div class="col-lg-4 col-md-6">
                        <div class="card border-0 rounded-0 text-center box_shadow1"
                             data-aos="{{ $key % 2 != 0 ? 'flip-left' : 'flip-right' }}">
                            <div class="card_header">
                                <h4 class="pb-30">@lang($plan->name)</h4>
                                <span>
                                    <span class="price">{{ config('basic.currency_symbol') . $plan->price }}</span>
                                </span>
                            </div>
                            <div
