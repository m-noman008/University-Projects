...
function removeSpaceTagName($tagName)
{
...
function timeFormat($data)
{
    if (preg_match("/^(2[0-3]|[01]?[0-9]):([0-5]?[0-9])$/", $data)) {
        $data = \Carbon\Carbon::createFromFormat('H:i', $data)->format('h:i A');
        return $data;
    } elseif (preg_match("/^(2[0-3]|[01]?[0-9]):([0-5]?[0-9]):([0-5]?[0-9])$/", $data)) {
        $data = \Carbon\Carbon::createFromFormat('H:i:s', $data)->format('h:i A');
        return $data;
    } else {
        // Handle the case where $data is not in the expected format
        throw new \InvalidArgumentException("Invalid time format: $data");
    }
}
...
function basicControl()
{
    ...
    if (!$configure) {
        $configure =  \App\Models\Configure::firstOrCreate(['id' => 1]);
...
function getPaymentUrl($order){
    if($order->plan_purchase_id){
        return route('user.plan.purchase',$order->plan_purchase_id);
    }elseif($order->product_order_id){
        return route('user.checkout');
    }
