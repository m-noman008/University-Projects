...
class PaymentController extends Controller
{
    ...
    public function depositConfirm(Request $request)
    {
        ...
        if ($order->status != 0) {
            return redirect(getPaymentUrl($order))->with('error', 'Invalid Payment Request');
        }
    ...
    public function fromSubmit(Request  $request)
    {
        ...
        if ($data->status != 0) {
            return redirect(getPaymentUrl($data))->with('error', 'Invalid Payment Request');
        }
    ...
    public function success(Request $request)
    {
        ...
        if(isset($data)){
            return redirect()->route('user.order.confirm', $order_id)->with('success', 'Product Order successfully');
        }else if(isset($plan)){
            return redirect()->route('user.home')->with('success', 'Plan purchased successfully');
        }
     ...
     * @return $amount
     */
    static public function newFund(Request $request, $user, $gate, $charge, $final_amo, $amount, $plan_purchase_id = null, $orderId = null): Fund
    {
        $fund = new Fund();
        $fund->user_id = $user->id;
        $fund->plan_purchase_id = $plan_purchase_id;
        $fund->product_order_id = $orderId;
